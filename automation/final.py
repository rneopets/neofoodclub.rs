...
# Patches the six static LOGIT_* arrays directly into the Rust source that
# actually feeds the wasm/Python builds, rather than emitting a standalone
# fragment someone has to copy by hand. Everything else in that file (the
# imports, the model struct/impl, the #[cfg(test)] module) is hand-written
# and untouched - only the LOGIT_INTERCEPTS..LOGIT_IS_POS4 block is replaced.
TARGET = Path("../crates/core/src/models/multinomial_logit.rs")
BLOCK_PATTERN = re.compile(
    r"static LOGIT_INTERCEPTS: \[f64; 20\] = \[.*?\nstatic LOGIT_IS_POS4: \[f64; 20\] = \[.*?\n\];\n",
    re.DOTALL,
)


def splice_rust_constants(new_blocks: str) -> None:
    content = TARGET.read_text()
    if not BLOCK_PATTERN.search(content):
        raise RuntimeError(f"could not find existing LOGIT_* constant block in {TARGET}")
    TARGET.write_text(BLOCK_PATTERN.sub(lambda _: new_blocks, content, count=1))


print(f"Patching {TARGET} with freshly trained coefficients...")
splice_rust_constants(generate_rust(params))
